    ...
    <section class="content">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <c:if test="${sessionScope.Check.toString().equals('ManageReader_0')}">
                            <form role="form" method="post"
                                  action="${pageContext.request.contextPath}/SearchReader">
                            </c:if>
                            <c:if test="${sessionScope.Check.toString().equals('ManageReader_1')}">
                                <form role="form" method="post"
                                      action="${pageContext.request.contextPath}/SearchReader?status=1">
                                </c:if>
                                <div class="card-header">
                                    <c:if test="${sessionScope.Check.toString().equals('ManageReader_0')}">
                                        <h3 class="card-title">Danh sách sách những người đang mượn sách</h3>
                                    </c:if>
                                    <c:if test="${sessionScope.Check.toString().equals('ManageReader_1')}">
                                    ...
                                    <div class="card-tools" style="margin-right: 1px;">
                                        <div class="input-group input-group-sm" style="width: 200px;">
                                            <input type="text" name="data_search"
                                                   class="form-control float-right" placeholder="Tìm kiếm theo tên">
                                                ...
                                                <c:if test="${sessionScope.Check.toString().equals('ManageReader_0')}">
                                                    <td>
                                                        <button type="submit" class="btn btn-warning  btn-secondary"
                                                                onclick="location.href = '${pageContext.request.contextPath}/ConfirmReader?id=${reader.getId()}'">Đã trả</button>
                                                    </td>
                                                </c:if>
                                            </tr>
                                        </c:forEach>
